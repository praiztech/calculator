...
function backSpace() {
  ...
  if (input[input.length - 1] in operations) {
    ...
    if ((inputArr.length <= 9) && (inputArr.length % 2 === 1)) {
      document.getElementById(idArr[countt]).textContent = inputArr[indexx - 1] || inputArr[0];
    }
  } else {
    let len = input.length; //to check if input was shortened in the for loop
    for (let j = input.length - 1; j >= 0; j--) {
      if (input[j] in operations) {
        input = input.slice(0, j + 1);
        break;
      }
    }
    if (input.length === len) {
      input = ''; //delete the only operand in input
    }
    if (input) {
      let inputArr = createInputArr();
      document.getElementById("A1").textContent = inputArr[inputArr.length - 1];
      let countt = 1; //starting from A2
      let indexx;
      for (let k = inputArr.length - 2; k >= 0; k--) {
      ...
      if ((inputArr.length < 9) && (inputArr.length % 2 === 0)) {
        document.getElementById(idArr[countt]).textContent = inputArr[indexx - 1] || inputArr[0];
      }
    }
  }  
